...
def calculateFrequentRolls(numberOfRolls):
    ...
    outcomeCounts = {}
    for count in range(2,DIE_SIDES*2+1):
    ...
    mostFrequentRolls,highestCount = findOutcomes(outcomeCounts)
...
def findOutcomes(outcomeCounts):
    """Returns a list of outcomes with the highest count as well as the highest count"""
    mostFrequentRolls = []
    highestCount = max(outcomeCounts.values())
    for key, value in outcomeCounts.items():
        if value == highestCount:
            mostFrequentRolls.append(key)
    return mostFrequentRolls, highestCount
